...
function install_package(){
    ...
    if(!$core->db->isFieldExists('groups', 'join_roles')){
        $core->db->query("ALTER TABLE `{#}groups` ADD `join_roles` VARCHAR(1000) NULL DEFAULT NULL COMMENT 'Ролі при вступі в групу'");
    }
    ...
    if(!$core->db->getRowsCount('scheduler_tasks', "controller = 'queue' AND hook = 'run_queue'")){
        $core->db->query("INSERT INTO `{#}scheduler_tasks` (`title`, `controller`, `hook`, `period`, `is_active`) VALUES ('Виконує задачі системної черги', 'queue', 'run_queue', '1', '1');");
    }
